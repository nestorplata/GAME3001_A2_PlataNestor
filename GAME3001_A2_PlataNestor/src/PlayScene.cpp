...
void PlayScene::start()
{
	...
	m_pImpassableTiles.push_back(m_getTile(4, 3));
	m_pImpassableTiles.push_back(m_getTile(4, 4));
	m_pImpassableTiles.push_back(m_getTile(4, 5));
	if (!m_pImpassableTiles.empty())
	{
		for (int i = 0; i<m_pImpassableTiles.size(); i++)
		{
			m_pImpassableTiles[i]->setTileStatus(IMPASSABLE);
